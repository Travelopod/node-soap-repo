...
export class Client extends EventEmitter {
  ...
  private _initializeOptions(options: IOptions) {
    ...
    this.wsdl.options.headerAttributes = options.headerAttributes || {};
  ...
  private _invoke(
    ...
    const headerAttributes = Object.keys(this.wsdl.options.headerAttributes)
      .map(
        (attribute) =>
          `${attribute}="${this.wsdl.options.headerAttributes[attribute]}"`
      )
      .join(" ");
